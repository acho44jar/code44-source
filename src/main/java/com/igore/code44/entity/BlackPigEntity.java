...
public class BlackPigEntity extends Pig {
    private int attackCooldownTicks;

    public BlackPigEntity(EntityType<? extends Pig> entityType, Level level) {
    ...
    public void tick() {
        ...
        this.getNavigation().moveTo(nearestPlayer.getX(), nearestPlayer.getY(), nearestPlayer.getZ(), 1.7D);
        this.getLookControl().setLookAt(nearestPlayer, 60.0F, 60.0F);

        if (this.attackCooldownTicks <= 0 && this.distanceToSqr(nearestPlayer) <= 2.3D * 2.3D) {
            nearestPlayer.hurt(this.level().damageSources().mobAttack(this), 4.0F);
            this.attackCooldownTicks = 18;
        }
    }
}
